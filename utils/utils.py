...
def run_evaluation(model, loader, device, save_dir, history=None, task="all"):
    ...
    with torch.no_grad():
        for ir, spec, t_det, t_dist, t_mat in loader:
            ...
            # Unpack 6 values (ignoring embeddings + aux_det during raw evaluation predictions)
            p_det, p_dist, p_mat, _, _, _ = model(ir, spec)
    ...
    if history is not None:
        ...
        # Plot Orthogonality Loss History Curve
        if "ortho_loss" in history and len(history["ortho_loss"]) > 0:
            plt.figure(figsize=(8,5))
            plt.plot(epochs, history["ortho_loss"], label="Ortho Loss", color="purple")
            plt.xlabel("Epoch")
            plt.ylabel("Loss")
            plt.title("Orthogonality Loss Curve")
            plt.legend()
            plt.grid()
            plt.savefig(save_dir/"ortho_loss_curve.png", dpi=300)
            plt.close()
...
def epoch_metrics(model, loader, device, task="all"):
    ...
    with torch.no_grad():
        ...
        for ir, spec, t_det, t_dist, t_mat in loader:
            ...
            # Unpack 5 values from forward pass
            p_det, p_dist, p_mat, _, _, _ = model(ir, spec)
