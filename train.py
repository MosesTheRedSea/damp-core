...
class MultiTaskLoss(nn.Module):
    ...
    def __init__(self, w_det=1.0, w_dist=1.0, w_mat=1.0, 
        ...
        if det_weights is not None:
            self.register_buffer('det_weights', det_weights)
        else:
            self.det_weights = None

    def forward(self, p_det, t_det, p_dist, t_dist, p_mat, t_mat, 
                e_geo=None, e_sem=None, aux_det=None):

        total = 0.0
        losses = {"det": 0.0, "dist": 0.0, "mat": 0.0}

        if p_det is not None and t_det is not None:
            loss_det = F.cross_entropy(p_det, t_det, weight=self.det_weights, label_smoothing=0.1)
            losses["det"] = (loss_det / math.log(self.num_det_classes)).item()
            total += self.w_det * (loss_det / math.log(self.num_det_classes))

        if aux_det is not None and t_det is not None:
            loss_aux = F.cross_entropy(aux_det, t_det, weight=self.det_weights, label_smoothing=0.1)
            losses["aux_det"] = (loss_aux / math.log(self.num_det_classes)).item()
            total += 0.3 * self.w_det * (loss_aux / math.log(self.num_det_classes))

        if p_mat is not None and t_mat is not None:
        ...
        if p_dist is not None and t_dist is not None:
        ...
        return total, (losses["det"], losses["dist"], losses["mat"], 0.0)
...
if __name__ == '__main__':
    ...
    for folder in train_folders:
        ...
        det_counts[det_map[meta["object_type"]]] += 1

    total_train = len(train_folders)

    det_weights = torch.ones(len(OBJ_CLASSES), dtype=torch.float32)
    for cls_idx, count in det_counts.items():
        det_weights[cls_idx] = total_train / (len(OBJ_CLASSES) * count)

    det_weights = det_weights / det_weights.mean()  # mean = 1.0
    det_weights = det_weights.to(DEVICE)

    print("\nDetection class weights:")
    for name, idx in sorted(det_map.items(), key=lambda x: x[1]):
        print(f"  {name:25s}: {det_weights[idx]:.3f}  (n={det_counts[idx]})")
    ...
    model = DAMP(len(OBJ_CLASSES), len(MAT_CLASSES), task=args.task, use_cross_attn=not args.no_cross_attn, 
        temporal_only=args.temporal_only, use_se=args.use_se).to(DEVICE)
    ...
    #     w_mat = 1.0,
    #     ortho_lambda=(
    #         0.0 if args.no_orth else 0.0001
    #     ),
    #     num_det_classes=len(OBJ_CLASSES),
    #     num_mat_classes=len(MAT_CLASSES),
    #     max_dist=MAX_DIST
    ...
    for epoch in range(EPOCHS):
        ...
        train_losses, train_det, train_dist, train_mat, train_ortho = [], [], [], [], []
        for ir, spec, t_det, t_dist, t_mat in train_loader:
            ...
            p_det, p_dist, p_mat, e_geo, e_sem, aux_det = model(ir, spec)

            loss, (l_det, l_dist, l_mat, l_aux) = criterion(
                p_det, t_det,
                p_dist, t_dist,
                p_mat, t_mat,
                e_geo=e_geo,
        ...
        with torch.no_grad():
            for ir, spec, t_det, t_dist, t_mat in val_loader:
                ...
                p_det, p_dist, p_mat, e_geo, e_sem, _ = model(ir, spec)

                v_loss, (vl_det, vl_dist, vl_mat, vl_aux) = criterion(
                    p_det, t_det,
                    p_dist, t_dist,
                    p_mat, t_mat,
                    e_geo=e_geo,
        ...
        history["ortho_loss"].append(np.mean(train_ortho))

        scheduler.step()

        print(
            f"Epoch [{epoch+1:3d}/{EPOCHS}] "
            f"Train: {avg_train:.4f} | Val: {avg_val:.4f} | SmoothedVal: {smoothed_val:.4f} | "
            f"Det: {det_acc:.1f}% | Mat: {mat_acc:.1f}% | "
            f"RMSE: {rmse:.3f}m | MAE: {mae:.3f}m | "
            f"TrainLoss=({avg_train_det:.3f},{avg_train_dist:.3f},{avg_train_mat:.3f},{np.mean(train_ortho):.4f}) | "
